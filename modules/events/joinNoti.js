...
module.exports.run = async function({ api, event, Users }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Edited by Beruss" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		return api.sendMessage(`Hãy tôn trọng và đọc kĩ hưỡng dẫn dùng bot! Cảm ơn bạn đã lựa chọn LOLIBOT =)`, threadID);
	}
	else {
		try {
